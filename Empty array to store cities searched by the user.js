...
function ajaxGetWeather(city){
    ...
    if (city != null){
        Query URL variables
    var currentWeatherQuery = `httpsapi.openweathermap.orgdata2.5weatherq=${city}&APPID=af6d8ce77f31fadf19d5245c0880c1e8`;
    var fiveDayWeatherQuery = `httpsapi.openweathermap.orgdata2.5forecastq=${city},3166&APPID=af6d8ce77f31fadf19d5245c0880c1e8`;
    ...
    $.ajax({
        ...
        var getDate = moment();
        var todaysDate = getDate.format(MMDDYYYY);

         Converting temp from Kelvin to Fahrenheit
        var temp = Math.round(((response.main.temp - 273.15)  9  5) + 32);
        $(#cityName).html(`${response.name} (${todaysDate}) spanimg src=httpsopenweathermap.orgimgwn${response.weather[0].icon}@2x.pngspan`);
    ...
    $.ajax({
        ...
        Displaying five day dates using getMonthgetDategetFullYear methods
        ...
        $(#iconDay1).attr(src, `httpsopenweathermap.orgimgwn${response.list[2].weather[0].icon}@2x.png`);
        $(#iconDay2).attr(src, `httpsopenweathermap.orgimgwn${response.list[10].weather[0].icon}@2x.png`);
        $(#iconDay3).attr(src, `httpsopenweathermap.orgimgwn${response.list[18].weather[0].icon}@2x.png`);
        $(#iconDay4).attr(src, `httpsopenweathermap.orgimgwn${response.list[26].weather[0].icon}@2x.png`);
        $(#iconDay5).attr(src, `httpsopenweathermap.orgimgwn${response.list[34].weather[0].icon}@2x.png`);
...
function retrieveUVIndex(response) {
    ...
    var uvIndexQuery = `httpsapi.openweathermap.orgdata2.5uviappid=af6d8ce77f31fadf19d5245c0880c1e8&lat=${response.coord.lat}&lon=${response.coord.lon}`;
